...
class HeaderV1:
    ...
    def from_bytes(cls, b: bytes, endian: str = "big") -> "CSP.HeaderV1":
        ...
        return cls(src, dst, dport, sport, prio=prio, flags=flags, endian=endian)
...
class Packet:
    ...
    def __init__(
        ...
        self.xtea_engine = (
            None
            if xtea_key is None
            else XTEAEngine(xtea_key, legacy_unsafe=xtea_legacy_unsafe)
        )
    ...
    def encode(self) -> bytes:
        ...
        if self.header.xtea and self.xtea_engine:
            ...
            payload = self.xtea_engine.encrypt(self.payload, nonce) + nonce.to_bytes(
                4, "big"
            )
