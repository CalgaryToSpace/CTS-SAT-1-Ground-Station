...
DuckDB is capped in code rather than by env var, at 500 MB and one thread
(`common.py`) -- it otherwise sizes both from the *host*, not the
container.

On top of those, `docker-compose.yml` sets container limits (`cpus`,
`mem_limit`, `memswap_limit`, `cpu_shares`) as a backstop, so a dependency
that sizes a pool from the host's core count behind our back still can't
take the whole box. `web` deliberately gets no `cpus` cap and twice the
`cpu_shares`, so it wins whenever both want the CPU.
