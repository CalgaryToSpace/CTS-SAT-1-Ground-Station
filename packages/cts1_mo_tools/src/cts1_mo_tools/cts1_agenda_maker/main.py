...
def _lat_lon_to_country(latitude: float, longitude: float) -> str | None:
    ...
    row = pl.DataFrame({"lat": [latitude], "lon": [longitude]}).select(
        find_closest_country("lat", "lon")
    )
    country_code = row.item()
    gs_country_obj = pycountry.countries.get(alpha_2=country_code)
    x = gs_country_obj.name if gs_country_obj else None

    end_time = time.perf_counter()
    print(f"lat_lon_to_country: {end_time - start_time:.3f}")
